...
def get_gauss_estimates(t_samples, y_samples, Y_max, window_size, sampling_dt, n, delay, nderivs, integration_dt):
    ...
    for d in d_list:
        lp, li = generate_lagrange(d=d, N=window_size, sampling_dt=sampling_dt)
    ...
    for i in range(window_size-1, num_samples):
        ...
        for j in range(nderivs):
            ...
            y_hat[j, (i-delay-1)*num_steps_per_sample : (i-delay)*num_steps_per_sample+1] = estimator.differentiate(t, j)
            # y_bound[j, (i-delay-1)*num_steps_per_sample : (i-delay)*num_steps_per_sample+1]  = np.abs(res_pol.deriv(j)(t))+deriv_bound(k=j, d=max(j,1), M=Y_max[d+1]+G_max, delta_s=sampling_dt)
